...
class EWayPaymentProvider(models.Model):
    ...
    def _eway_encrypt_credentials(self, endpoint, payload=None):
        ...
        try:
            response = requests.post(eway_url, headers=headers, data=json.dumps(payload), auth=(self.eway_public_api_key, self.eway_password))
    ...
    def _eway_make_request(self, endpoint, payload=None, method='POST', offline=False):
        ...
        try:
            if method == 'POST':
                response = requests.post(eway_url, headers=headers, data=json.dumps(payload), auth=(self.eway_api_key, self.eway_password))
            elif method == 'GET':
